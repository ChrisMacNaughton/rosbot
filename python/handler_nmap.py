...
#For verbose output of the scripts execution. Used for debugging purposes. obviously... 
DEBUG = False
...
target_host = sys.argv[2] if re.match(r'(?!-|\.)[a-zA-Z\d\-\./]{1,63}(?<!-|\.)$',sys.argv[2]) else None
target_project = sys.argv[3] if re.match(r'[a-zA-Z\d\-_]+$',sys.argv[3]) else None

#setting up nmap scan profiles
args_quick = ["sudo","nmap","-sS","-sV","-Pn","--open","-v",target_host]
args_full = ["sudo","nmap","-sS","-sV","-sC","-Pn","-p0-","--open", "-v",target_host]
...
#for debug messages. stderr.write pushes output to RC during the script's runtime
def debug(string):
 if DEBUG:
  ...
  if nmap_file:
   ...
   if "scan initiated" in header:
    ...
    if target_key in scopes.keys():
      scopes[target_key] = temp_dir_for_git+'/source/scans/nmap/'+nmap_file[0]
    else:
     scopes[target_key] = temp_dir_for_git+'/source/scans/nmap/'+nmap_file[0]
...
#for tranlation of room's ID (on which the nmap command was called) into plain-text name. if the room starts with 'pen-', then it is used for defining the target repo
def get_room_name(roomId):
 login_data = urllib.urlencode({'username':rosbot_username,'password':rosbot_password})
 ...
 try:
  ...
  if error.code == 400:
   ...
   debug('Unknown error occured while retrieving RC channel name. HTTP error code: '+str(error.code))
   return None

#verfying the existence of optionally supplied project name. Allows to launch the nmap scan for a specific project from any channel/room.
...
#tryting to resolve the target project's name via room's name or supplied project's name. 'nosave' parameter can be used to bypass the check and run a scan without pushing stuff to gitlabs
if target_project != 'nosave':
 ...
 if roomName is not None and roomName.startswith('pen-'):
  ...
  debug('Project name derived from the supplied project name paramter in the command.')
 else:
  target_project = None
...
if target_project != 'nosave':
 ...
 debug('git clone git@'+gitlab_server+':'+gitlab_namespace+'/'+target_project+'.git '+temp_dir_for_git)
 proc = subprocess.Popen(['git', 'clone', 'git@'+gitlab_server+':'+gitlab_namespace+'/'+target_project+'.git', temp_dir_for_git],stdout=DEVNULL, stderr=DEVNULL)
 proc.wait()
 ...
 shutil.copytree(output_temp,temp_dir_for_git+'/source/scans/nmap/'+time_stamped_dir+'_'+scan_type)
 ...
 for item in scan_items:
  ...
  args = list(args_full) if "/full" in scan_items[item] else list(args_quick)
  args[-1] = target
 ...
 proc = subprocess.Popen(['git', 'add', 'source/scans'], stdout=DEVNULL, stderr=DEVNULL)
 proc.wait()#wait until the command finishes
 proc = subprocess.Popen(['git', 'commit', '-m', 'Automatic scan upload'],stdout=DEVNULL, stderr=DEVNULL)
 proc.wait()#wait until the command finishes
 proc = subprocess.Popen(['git', 'push', '-u', 'origin', 'master'],stdout=DEVNULL, stderr=DEVNULL)
 proc.wait()#wait until the command finishes
